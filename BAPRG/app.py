...
def run_ba_exam_app():
    # Load subjects_marathi from BAPRG directory dynamically
    current_dir = os.path.dirname(os.path.abspath(__file__))
    subjects_path = os.path.join(current_dir, 'subjects_marathi.py')
    spec = importlib.util.spec_from_file_location("subjects_marathi_ba", subjects_path)
    ...
    uploaded_file = st.file_uploader("Choose a DBF, Excel, or CSV file", type=["dbf", "xlsx", "csv"], key="ba_file_uploader")
    ...
    if uploaded_file:
        ...
        try:
            ...
            # Rule 2: Exclude if RES or FREM is not blank
            for col in ["RES", "FREM"]:
            ...
            with st.expander("Subject Column Mapping (Verify & Change)", expanded=True):
                ...
                sub1_col_selected = cols[0].selectbox("Select Column for SUB1 (P1_CD default)", ["-- None --"] + list(df.columns), index=default_sub1_idx, key="ba_sub1_select")
                sub2_col_selected = cols[1].selectbox("Select Column for SUB2 (P4_CD default)", ["-- None --"] + list(df.columns), index=default_sub2_idx, key="ba_sub2_select")

            # --- Manual Entries ---
            st.write("### Fixed Values")
            faculty_value = st.selectbox("Select Faculty", faculty_options, key="ba_faculty_select")
